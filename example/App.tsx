...
import { Alert, Button, SafeAreaView, ScrollView, Text, View, StyleSheet, Image, Dimensions, TouchableOpacity } from 'react-native';
import * as DocumentPicker from 'expo-document-picker';
import * as FileSystem from 'expo-file-system';
import * as VideoThumbnails from 'expo-video-thumbnails';
import { Asset } from 'expo-asset';
import ExpoObjectTracker, { BoundingBox, TrackingData, EffectConfig, PreviewFrame } from 'expo-object-tracker';
...
export default function App() {
  ...
  const [currentPreviewEffect, setCurrentPreviewEffect] = useState<EffectConfig | null>(null);
  ...
  const [detectedObjectPreview, setDetectedObjectPreview] = useState<string>('');
  ...
  const onBoundingBoxSelected = async (boundingBox: BoundingBox) => {
    try {
      ...
      const previewUri = await ExpoObjectTracker.generateObjectPreview(videoUri, id);
  ...
  const trackObject = async () => {
    ...
    try {
      ...
      const frames = await ExpoObjectTracker.generatePreviewFrames(videoUri, results, 5);
  ...
  // 효과 미리보기
  const previewEffect = async (effectConfig: EffectConfig, effectName: string) => {
    ...
    try {
      ...
      Alert.alert('Error', `Failed to generate ${effectName} preview: ` + error);
  ...
  // 각 효과별 미리보기 핸들러들
  const previewBlurEffect = () => previewEffect({ type: 'blur', intensity: 8 }, 'blur');
  const previewMosaicEffect = () => previewEffect({ type: 'mosaic', blockSize: 15 }, 'mosaic');
  const previewEmojiEffect = () => previewEffect({ type: 'emoji', emoji: '😎', scale: 1.5 }, 'emoji');
  const previewColorEffect = () => previewEffect({ type: 'color', color: '#FF0000', opacity: 0.7 }, 'color');
          ...
          {selectedBoundingBox && (
            <View style={styles.detectionInfo}>
              <Text style={styles.detectionText}>
                인식된 영역 크기: {Math.round(selectedBoundingBox.width)} × {Math.round(selectedBoundingBox.height)}
              </Text>
              <Text style={styles.detectionText}>
                위치: ({Math.round(selectedBoundingBox.x)}, {Math.round(selectedBoundingBox.y)})
              </Text>
            </View>
          )}
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.container}>
        ...
        <Group name="1. 비디오 선택">
          ...
          {videoUri ? <Text style={styles.infoText}>✓ 비디오 로드됨</Text> : null}
        </Group>
            ...
            <View>
              ...
              {selectedBoundingBox && (
                <Text style={styles.resultText}>
                  위치: ({Math.round(selectedBoundingBox.x)}, {Math.round(selectedBoundingBox.y)}) 
                  크기: {Math.round(selectedBoundingBox.width)}×{Math.round(selectedBoundingBox.height)}
                </Text>
              )}
            </View>
          ) : null}
        ...
        <Group name="3. 객체 추적">
          ...
          {trackingResults.length > 0 ? (
            <Text style={styles.infoText}>✓ {trackingResults.length}프레임에서 추적됨</Text>
          ) : null}
        </Group>
